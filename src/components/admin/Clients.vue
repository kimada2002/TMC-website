...
const handleImageUpload = async (event) => {
  ...
  try {
    // Upload to Storage
    const fileRef = storageRef(storage, `clients/${Date.now()}-${file.name}`);
    await uploadBytes(fileRef, file);
    const downloadUrl = await getDownloadURL(fileRef);

    // Save to Firestore
    await addDoc(clientsCollection, { url: downloadUrl });
...
const deleteImage = async (imageId) => {
  try {
    ...
    const imageDocRef = doc(db, "clients", imageId);
    ...
    const filePath = decodeURIComponent(
      imageUrl.split("clients%2F")[1].split("?")[0]
    );

    // Delete from Storage
    const fileRef = storageRef(storage, `clients/${filePath}`);
